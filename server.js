...
// CORS - V4 FIX: soporta * y *.vercel.app
const ALLOWED_ORIGINS = (process.env.ALLOWED_ORIGINS || '').split(',').map(s=>s.trim()).filter(Boolean);
app.use(cors({
  origin: (origin, cb) => {
    if (!origin) return cb(null, true);
    // si no hay lista, permite todo (dev)
    ...
    for(const allowed of ALLOWED_ORIGINS){
      if(allowed.startsWith('*.') && origin.endsWith(allowed.slice(1))) return cb(null, true);
      if(allowed === '*.vercel.app' && origin.endsWith('.vercel.app')) return cb(null, true);
    }
    // si ALLOWED_ORIGINS contiene vercel.app, permite cualquier subdominio vercel
...
// CORS - FIXED: strict when ALLOWED_ORIGINS set
const ALLOWED_ORIGINS = (process.env.ALLOWED_ORIGINS || '').split(',').map(s=>s.trim()).filter(Boolean);
app.use(cors({
  origin: (origin, cb) => {
    if (!origin) return cb(null, true);
